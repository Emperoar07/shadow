...
// ── Inline types (avoids importing from Next.js app) ──────────────────────────
...
// Re-export ShadowPerpClient from the app workspace
// The relay server is run from the repo root so this path resolves correctly
export { ShadowPerpClient } from "../../app/src/lib/client";

// ── Trading pairs (inline to avoid importing from app) ────────────────────────
...
function buildRelayConfig(programId: PublicKey): ShadowPerpConfig {
  ...
  return {
    ...
    marketAddress:  parsePublicKey("SHADOWPERP_MARKET_ACCOUNT", (marketRegistry["SOL-USD"] as PublicKey).toBase58()),
    marketRegistry,
...
export type RelayRuntimeContext = {
  client: any; // ShadowPerpClient — typed as any to avoid circular import issues at build time
...
export async function createRelayRuntimeContext(): Promise<RelayRuntimeContext> {
  ...
  // Dynamically import ShadowPerpClient to avoid build-time issues
  const { ShadowPerpClient } = await import("../../app/src/lib/client");
  return {
