    ...
    <div className="trade-trading-panel flex flex-col bg-shadow-900 p-2 h-full overflow-y-auto">
      <div className={isHorizontal ? "grid grid-cols-1 items-start gap-1.5 lg:grid-cols-12" : "space-y-1"}>
        <div className={isHorizontal ? "space-y-1 lg:col-span-12" : "space-y-1"}>
          ...
          {/* No margin balance warning */}
          {publicKey && marginBalance !== null && marginBalance <= 0 && (
            <button
              type="button"
              onClick={() => setCollateralModalOpen(true)}
              className="w-full rounded-lg border border-yellow-500/30 bg-yellow-500/8 px-3 py-2 text-[11px] text-yellow-400 hover:bg-yellow-500/15 transition-colors flex items-center justify-between gap-2"
            >
              <span className="flex items-center gap-1.5">
                <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" />
                </svg>
                No margin balance — deposit USDC to open positions
              </span>
              <span className="font-semibold shrink-0">Deposit →</span>
            </button>
          )}
