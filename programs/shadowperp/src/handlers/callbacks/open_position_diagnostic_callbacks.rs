...
const COMP_DEF_OFFSET_OPEN_POSITION_TUPLE_PROBE_U8_V1: u32 =
    comp_def_offset("open_position_tuple_probe_u8_v1");
...
#[callback_accounts("open_position_tuple_probe_u8_v1")]
#[derive(Accounts)]
pub struct OpenPositionTupleProbeU8V1Callback<'info> {
    pub arcium_program: Program<'info, Arcium>,
    #[account(address = derive_comp_def_pda!(COMP_DEF_OFFSET_OPEN_POSITION_TUPLE_PROBE_U8_V1))]
    pub comp_def_account: Account<'info, ComputationDefinitionAccount>,
    #[account(address = derive_mxe_pda!())]
    pub mxe_account: Account<'info, MXEAccount>,
    /// CHECK: Validated by Arcium
...
pub fn open_position_tuple_probe_u8_v1_callback_handler(
    ...
    let verified_output = match output.verify_output(
        ...
        Err(error) => {
            msg!(
                "tuple u8 probe verify failed for diagnostic {}: {}",
                diagnostic.key(),
                error
            );
            abort_probe(diagnostic, ctx.accounts.computation_account.key(), clock.unix_timestamp)?;
    ...
    require!(
        ctx.accounts.comp_def_account.key()
            == derive_comp_def_pda!(COMP_DEF_OFFSET_OPEN_POSITION_TUPLE_PROBE_U8_V1),
        ShadowPerpError::Unauthorized
    );
    require!(
        diagnostic.pending_computation_account == ctx.accounts.computation_account.key(),
